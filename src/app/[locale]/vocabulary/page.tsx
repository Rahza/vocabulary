    ...
    >
      <header className="flex justify-between items-start">
        <motion.div variants={itemReveal}>
          <Heading level={1} subtitle={t('subtitle', { count: vocabulary.length })}>
            {t('title')}
          </Heading>
        </motion.div>
        <motion.div variants={itemReveal}>
          <Link href="/tags">
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl border-2 font-black gap-2 h-10 px-4 mt-1"
            >
      ...
      <Drawer isOpen={!!detailItem} onClose={() => setDetailItem(null)} title={t('detailTitle')}>
        {detailItem && (
          <WordDetail item={detailItem} onRefresh={loadData} onClose={() => setDetailItem(null)} />
        )}
      </Drawer>
