...
export const PracticeConfig = ({ mode, onStart, onBack }: PracticeConfigProps) => {
  ...
  if (loading)
    return (
      <div className="text-center py-10 font-bold text-zinc-400 animate-pulse uppercase tracking-widest">
        {commonT('loading')}
      </div>
    );
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-4">
        <h3 className="text-sm font-black uppercase tracking-widest text-zinc-400 ml-1">
          {t('sessionLength')}
        </h3>
      ...
      <div className="pt-4 flex gap-3">
        <Button
          onClick={onBack}
          variant="outline"
          className="flex-1 h-16 rounded-3xl border-2 font-black"
        >
          {commonT('back')}
        </Button>
        <Button
