...
export function GeneratedList({ items, onSave, onDiscard, onDelete }: GeneratedListProps) {
  ...
  return (
    <motion.div variants={containerReveal} initial="hidden" animate="visible" className="space-y-6">
      {items.length > 0 && (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {item.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 rounded-full"
                  >
                    #{tag}
                  </span>
                ))}
      ...
      {items.length > 0 && (
        <div className="flex gap-4 sticky bottom-24 bg-gray-50/80 dark:bg-zinc-950/80 backdrop-blur-md p-2 rounded-[32px]">
          <Button
            variant="outline"
            onClick={onDiscard}
            className="flex-1 h-16 rounded-3xl border-4 font-black"
          >
            {t('discard')}
          </Button>
          <Button
            onClick={onSave}
            variant="playful"
            className="flex-1 h-16 rounded-3xl border-b-4 font-black"
          >
            {t('save')}
          </Button>
        </div>
